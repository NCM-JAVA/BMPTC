                    ...
                    <div class="row">
                        <div class="col-md-8">
                            ...
                            <div class="mb-3 row align-items-center">
							 ...
							 <div class="mb-3 row align-items-center">
    <label for="St_pdf" class="col-sm-4 col-form-label">
        <b>Upload PDF</b>
    </label>
    <div class="col-sm-6">
        {{-- File input --}}
        <input type="file" name="St_pdf" id="St_pdf" class="form-control" accept="application/pdf">

        {{-- Show current PDF if it exists --}}
        @if(!empty($data->St_pdf))
            <div class="mt-2">
                <a href="{{ asset('public/assets/uploads/pdf/state/' . $data->St_pdf) }}"
                   target="_blank"
                   class="btn btn-sm btn-warning me-1">
                   <i class="bi bi-file-earmark-pdf-fill"></i> View Current PDF
                </a>
               
            </div>
        @else
            <div class="mt-2 text-muted">No PDF uploaded yet.</div>
        @endif
    </div>
</div>
<div class="mb-3 row align-items-center">
    <label for="risk_pdf" class="col-sm-4 col-form-label">
        <b>Upload Risk PDF</b>
    </label>
    <div class="col-sm-6">
        {{-- File input --}}
        <input type="file" name="risk_pdf" id="risk_pdf" class="form-control" accept="application/pdf">

        {{-- Show current PDF if it exists --}}
        @if(!empty($data->risk_pdf))
            <div class="mt-2">
                <a href="{{ asset('public/assets/uploads/pdf/state/' . $data->risk_pdf) }}"
                   target="_blank"
                   class="btn btn-sm btn-warning me-1">
                   <i class="bi bi-file-earmark-pdf-fill"></i> View Current PDF
                </a>
                            ...
                            @if($data->image)
                                <img src="{{ asset('public/assets/img/uploads/state/' . $data->image) }}" style="{{ !empty($data->image) ? 'cursor:pointer;' : 'cursor:default;' }}" data-bs-toggle="modal"
									...
									<a href="{{ route('admin.file.delete', [
                                        ...
                                        'id' => $data->id,
                                        'column' => 'image',
                                        'path' => 'assets/uploads/img/uploads/state'
